...
# Provider calls retry only TRANSIENT API errors (network, rate limit, 5xx/
# overload) with exponential backoff, this many attempts total. A permanent error
# such as a 400 out-of-credits is never retried.
_MAX_STREAM_ATTEMPTS = 3
_MAX_JSON_ATTEMPTS = 3


def _is_transient(err: Exception) -> bool:
    """True for API failures worth retrying: connection/timeout, rate limit, and
    server-side 5xx/overload. A 4xx (e.g. a 400 out-of-credits) is permanent."""
    if isinstance(
        err,
        (
...
class AnthropicProvider:
    ...
    async def stream(
        ...
        *,
        ...
        # Retry transient failures (network, rate limit, 5xx/overload) with
        # exponential backoff, but ONLY while nothing has been streamed yet:
        # retrying after tokens have gone out would double-send them. Permanent
        # errors (e.g. a 400 out-of-credits) are never retried.
        for attempt in range(1, _MAX_STREAM_ATTEMPTS + 1):
            ...
            produced = False  # any token/tool emitted → cannot safely retry
            ...
            try:
                ...
                if _is_transient(e) and not produced and attempt < _MAX_STREAM_ATTEMPTS:
                    delay = 2 ** (attempt - 1) + random.random()
                    logger.warning(
                        "stream transient error (attempt %d/%d) — retrying in %.1fs: %s",
                        attempt, _MAX_STREAM_ATTEMPTS, delay, e,
                    )
        ...
        *,
        ...
        for attempt in range(1, _MAX_JSON_ATTEMPTS + 1):
            try:
                ...
                # Retry only transient failures with exponential backoff; a
                # permanent error (e.g. a 400 out-of-credits) will not fix itself
                # inside this loop, so fail fast and return None.
                if _is_transient(e) and attempt < _MAX_JSON_ATTEMPTS:
                    delay = 2 ** (attempt - 1) + random.random()
                    logger.warning(
                        "complete_json transient error (label=%s attempt %d/%d) — retrying in %.1fs: %s",
                        label or "-", attempt, _MAX_JSON_ATTEMPTS, delay, e,
                    )
                    await asyncio.sleep(delay)
                    continue
...
def get_provider() -> LLMProvider:
    if settings.llm_provider == "anthropic":
        # Prefer a key saved at runtime via the Settings panel; fall back to the
        # .env key when none has been saved.
        api_key = runtime_config.load_api_key() or settings.anthropic_api_key
